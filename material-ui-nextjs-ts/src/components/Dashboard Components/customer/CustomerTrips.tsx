...
export default async function Trips({data}: {data:any}) {
    ...
    const columns = [
        { id: "carID", label: "Car", minWidth: 170 },
        { id: "dateCreated", label: "Date", minWidth: 170 },
        { id: "time", label: "Time", minWidth: 170 },
      ];
    
      const rows = data.map((item: { carID: string; dateCreated: string | number | Date; timeBegin: string | number | Date; timeEnd: string | number | Date; }) => ({
        carID: "Gyro #" + item.carID,
